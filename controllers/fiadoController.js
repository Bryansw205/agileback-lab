...
exports.marcarPagado = (req, res) => {
  ...
  const query = `UPDATE fiado SET estado_pago = 'pagado', fecha_cancelacion = NOW() WHERE id_venta = ?`;
...
exports.listarFiados = (req, res) => {
  const query = `
  SELECT f.id_venta, v.fecha AS fecha_fiado, CONCAT(c.nombre, ' ', c.apellidos) AS cliente, v.total AS monto, f.estado_pago, f.fecha_limite_pago,
